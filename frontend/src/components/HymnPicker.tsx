...
import { useHymnalMissing } from '@/lib/hymnal-status'
...
export function HymnPicker({ label, value, onChange, hint }: HymnPickerProps) {
  // ⛔ An empty result and a missing hymnal look identical to the reader (BUG-024).
  const hymnalMissing = useHymnalMissing()
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      // ⛔⛔ WITH NO HYMNAL THERE IS NOTHING TO ASK FOR, AND THE PANEL MUST STILL OPEN.
      // `setOpen(data.length > 0)` meant an empty index could never open the dropdown,
      // so the note explaining the emptiness had nowhere to render and typing produced
      // silence — the exact symptom BUG-024 is about.
      if (hymnalMissing) {
        setResults([])
        setOpen(true)
        setActiveIndex(-1)
  ...
  }, [query, updateDropDirection, hymnalMissing])
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {/* ⭐ SAY WHY IT IS EMPTY, AT THE MOMENT IT IS EMPTY. The notice on load is an
          announcement; this is the answer to "why did nothing come back", asked
          three fields later by someone who dismissed it. */}
      {open && hymnalMissing && (
        <div className={dropdownClasses}>
          <p className="px-3 py-2.5 text-sm text-muted-foreground">
            No hymnal is set up, so there is nothing to search.
            {/* ⭐ Naming the books matters more here than in the notice: this same panel
                appears under Creed and Liturgical Prayer, where "no hymnal" alone reads as
                the wrong explanation. */}
            <span className="block mt-1 text-xs">
              OpenOrder uses the United Methodist Hymnal and The Faith We Sing.
              Settings → Folders → Hymnal Folder.
            </span>
          </p>
        </div>
      )}
      {open && !hymnalMissing && results.length > 0 && (
        <div className={dropdownClasses}>
          {results.map((hymn, i) => (
            <button
