...
export function CalendarTab({ serviceDate, church }: { serviceDate: string; church: string }) {
  // ⛔⛔ EVERY CALENDAR CALL CARRIES THE CHURCH. The calendar is per congregation
  // now; without this, editing one church's calendar writes into whichever set of
  // files the server picked, which is how a second church ended up showing the
  // first one's events. @decision:gold 2026-09-24
  ...
  async function loadAll() {
    ...
    const [t, e, o] = await Promise.all([
      fetch(`/api/calendar/templates${q}`).then(r => r.json()),
      fetch(`/api/calendar/events${q}`).then(r => r.json()),
      fetch(`/api/calendar/overrides${q}`).then(r => r.json()),
    ])
    setTemplates(t)
    setEvents(e)
    setOverrides(o)
  ...
  async function loadNote() {
    if (!serviceDate) return
    const n = await fetch(`/api/calendar/note/${serviceDate}${q}`).then(r => r.json())
    setNote(n)
  }
  ...
  async function saveNote(updates: Partial<Note>) {
    ...
    setNote(newNote)
    await fetch(`/api/calendar/note/${serviceDate}${q}`, {
      method: 'POST',
  ...
  async function toggleSkip(templateId: string, date: string, currentlySkipped: boolean) {
    await fetch(`/api/calendar/overrides/skip${q}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ templateId, date, skip: !currentlySkipped })
    })
    const o = await fetch(`/api/calendar/overrides${q}`).then(r => r.json())
    setOverrides(o)
  }

  async function saveTemplate(t: Template) {
    if (t.id && templates.find(x => x.id === t.id)) {
      await fetch(`/api/calendar/templates/${t.id}${q}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(t)
      })
    } else {
      await fetch(`/api/calendar/templates${q}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(t)
      })
  ...
  async function deleteTemplate(id: string) {
    ...
    await fetch(`/api/calendar/templates/${id}${q}`, { method: 'DELETE' })
    loadAll()
  }

  async function saveEvent(e: Event) {
    if (e.id && events.find(x => x.id === e.id)) {
      await fetch(`/api/calendar/events/${e.id}${q}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(e)
      })
    } else {
      await fetch(`/api/calendar/events${q}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(e)
      })
  ...
  async function deleteEvent(id: string) {
    ...
    await fetch(`/api/calendar/events/${id}${q}`, { method: 'DELETE' })
    loadAll()
  }
