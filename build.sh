...
# executable inside the bundle by default; point it at a cloud-synced
# folder via the in-app data-directory setting (settings.json "data_dir").
#
...
# --- 0. Claim the build number FIRST ----------------------------------------
# ⛔⛔ THE BUMP MUST PRECEDE THE FRONTEND BUILD. Vite compiles the number into the
# app (frontend/vite.config.ts reads OO_BUILD_NUMBER), so bumping afterwards —
# which is what this script used to do — shipped every app stamped with the
# PREVIOUS build's number. Silently, and forever. The plist stamp in step 4 reads
# the same variable, so the bundle and the UI cannot disagree.
...
export OO_BUILD_NUMBER="$BUILD_NUMBER"
echo "  [0/6] Build number $BUILD_NUMBER (v$VERSION)"

# --- 1. Frontend ------------------------------------------------------------
echo "  [1/6] Building frontend ..."
( cd frontend && OO_BUILD_NUMBER="$BUILD_NUMBER" npm run build --silent )
echo "        Done."
...
"$VENV_PYTHON" -m PyInstaller --noconfirm --onedir --windowed \
    ...
    --add-data "bible-text:bible-text" \
...
# is exactly what you need when a report says "it's doing X" and you must know
# which binary is running.
echo "  [4/6] Stamping version ..."
# ⛔ VERSION and BUILD_NUMBER were claimed in step 0, before the frontend was
# compiled. Do NOT bump here — that is where the off-by-one lived.
...
# so quit it first: closing a running app beats a build stuck waiting on it.
#
# GRACEFUL QUIT ONLY, and narrow by name — stopping a process is
# destructive-tier. `quit app` lets any in-flight write to the user's data finish;
