...
def _resolve(key: str, default: Path, creatable: bool = True) -> Path:
    ...
    case: settings written on Windows named a `D:/...` folder, which on the Mac
    is simply nowhere, and without this check every save would fail.)
    """
...
def unique_path(path: Path) -> Path:
    """`path` if it is free, otherwise the next "name (2).ext" beside it.

    ⛔⛔ GENERATION NEVER OVERWRITES. @decision:gold 2026-09-24

    Rewriting a file in place is invisible in the one place the user actually
    looks: the Finder sorts Downloads by DATE ADDED, and an overwrite keeps the
    original's date added. So a deck regenerated five times stayed buried where
    the first one landed, and the folder looked as though nothing had been
    produced for half an hour. The file was correct every time; it simply could
    not be found.

    ⭐ A NEW NAME FIXES IT AT THE SOURCE — a new file gets a new date added, so it
    appears at the top whatever the sort column is. It also means a document open
    in Word or PowerPoint no longer blocks the next generation.
    """
    if not path.exists():
        return path
    ...
    for n in range(2, 1000):
        ...
        if not candidate.exists():
            return candidate
    # 998 copies of one name is not a case worth a clever answer, but silently
    # overwriting the original would be the worst possible one.
    from datetime import datetime
    return parent / f"{stem} ({datetime.now():%H%M%S}){suffix}"
