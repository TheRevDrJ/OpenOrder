...
│       ├── churches.py      # Church profiles — template, theme, defaults
│       ├── calendar_data.py # Calendar templates, events, overrides
│       └── slide_art.py     # Slide backdrop compositing
├── frontend/
│   └── src/
...
A **church profile** carries everything that differs between congregations — the Word template a bulletin is built from, the slide palette and typography, the artwork behind each slide section, and that church's own calendar.

Profiles live in your data folder rather than in the package, so nothing about your congregation ships with the source, and updating the app never touches them:

```
<data_dir>/churches/<id>/
├── church.json                        # palette, typography, defaults
├── Template - Bulletin.docx           # your letterhead and layout
├── Template - Bulletin - Communion.docx
├── images/<slot>.png                  # slide backdrops
└── calendar-*.json                    # this church's calendar
```

Colors are hex strings, and anything a profile leaves out falls back to a neutral default — so a fresh install works with nothing configured at all. Run more than one profile and each congregation gets its own template, palette, artwork and calendar from the same saved service.

If you'd like help building a profile from your church's existing bulletin, [reach out](https://github.com/TheRevDrJ/OpenOrder/issues) — we're happy to help.
...
- [x] Configurable data directory for shared/cloud-synced setups
