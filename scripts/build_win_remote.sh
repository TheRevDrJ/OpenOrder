...
#
# Modeled directly on FirstLight's script of the same name, which exists
# because a hand-composed leg lost a receipt to its own `tail -25` pipe. The
# lesson there is the one that governs here: a step that lives only in "how
# it is usually done" does not exist, because habits are not carried from one
# session to the next - only what is scripted happens every time.
#
# WHAT IT ENCODES:
...
LOGDIR="$HOME/claude-build/openorder-logs"   # outside the synced folder: regenerable
mkdir -p "$LOGDIR"
...
[ -z "$DIRT" ] || fail "uncommitted changes beyond the build's own side-effects - commit and push first (the build bundles the working tree, not HEAD):
$DIRT"
