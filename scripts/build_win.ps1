...
# the d:\claude\OpenOrder checkout. PowerShell, never .bat (.bat parsing is brittle).
#
...
# through the hymnal_dir setting instead, which on the test rig points at a
# hymnal-json folder on a synced drive outside the clone. Keeping it out of the
...
# ---- 1. frontend ----------------------------------------------------------
# The number is compiled into the app by vite (frontend/vite.config.ts reads
# OO_BUILD_NUMBER), so it must be in the environment BEFORE npm run build. This
# leg READS the event's number and never bumps it, so both platforms' artifacts
# carry the same one.
Write-Output '[1/4] Building frontend ...'
$env:OO_BUILD_NUMBER = $N
...
# bug when it is run. One archive means one SHA256 answers "did it arrive
