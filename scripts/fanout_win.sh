...
#
# A Windows build has two standing destinations, and both were being done by hand off a runbook on FirstLight until a build
...
# looks exactly like a code bug when it is run. Every copy is verified SHA256
# against the source on the build machine, and a mismatch aborts loudly rather
# than handing over something to waste an evening on.
#
...
#   Defiant = this Mac - the artifact is kept in hand here too
set -e
...
# fail with the previous build open on screen. Abort and SAY so - never kill
# a process on a machine someone might be using.
ssh "$PHX" "if (Test-Path '$PHX_DIR') { Remove-Item -Recurse -Force '$PHX_DIR' }; Expand-Archive -Path 'c:\\claude\\OpenOrder-win.zip' -DestinationPath '$PHX_DIR' -Force" \
  > /dev/null 2>&1 \
  || fail "could not unpack on phoenix. If OpenOrder is RUNNING there it holds its own exe open - close it there, then run this again."
